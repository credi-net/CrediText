...
class ExtractWetContentsJob(CCSparkJob):
    ...
    @staticmethod
    def is_domain_exist(
        domain_name: str, url='http://206.12.91.10:22101/searchDomainName/'
    ):
    ...
    def process_record(self, record):
        ...
        if self.is_wet_text_record(record):
            ...
            if WARC_Identified_Content_Language:
                WARC_Identified_Content_Languages_lst = (
                ...
                if (
                    len(
                        set(WARC_Identified_Content_Languages_lst)
                        & set(self.supported_langs)
                    )
                    > 0
                ):
    ...
    def run_job(self, session):
        ...
        self.domains_pc1_dict = session.sparkContext.broadcast(
            self.load_domain_pc1(self.args.trusted_domains)
        )
        ...
        w_desc = Window.partitionBy('Domain_Name').orderBy(
            F.col('Content_Length').desc()
        )
        w_asc = Window.partitionBy('Domain_Name').orderBy(F.col('Content_Length').asc())
        df_low = (
            df.withColumn('rn', F.row_number().over(w_asc))
            .filter(F.col('rn') <= 2)
            .drop('rn')
        )
        df_high = (
            df.withColumn('rn', F.row_number().over(w_desc))
            .filter(F.col('rn') <= 2)
            .drop('rn')
        )
        df_final = df_low.union(df_high).distinct()
        df_final = df_final.orderBy(
            F.col('Domain_Name').asc(), F.col('Content_Length').asc()
        )
        # df_sorted.collect()
        # df_sorted_Domain_Name_counts = df_final.groupBy("Domain_Name").count().collect()
        df_final.coalesce(self.args.num_output_partitions).write.format(
            self.args.output_format
        ).option('compression', self.args.output_compression).mode(
            'overwrite'
