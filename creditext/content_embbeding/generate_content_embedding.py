...
def get_ds_files(repo_id: str, pattern: str = '*.parquet') -> List[str]:
...
def main() -> None:
    root = get_root_dir()
    scratch = get_scratch()
    setup_logging(cast(str, root / 'embedding_generation.log'))
    parser = argparse.ArgumentParser(description='Content Embedding')
    parser.add_argument(
        '--hf_repo_id',
        type=str,
        default='Hussein-Abdallah/CrediBench-WebContent-Dec2024',
        help='dqr dataset path',
    )
    parser.add_argument(
        '--emb_model',
        type=str,
        ...
        choices=[
            ...
            'sentence-transformer/all-MiniLM-v2',
        ],
    )
    parser.add_argument(
        '--hf_files_start_idx', type=int, default=0, help='start file idx'
    )
    parser.add_argument(
        '--hf_files_end_idx', type=int, default=100, help='end file idx'
    )
    parser.add_argument(
        '--parquet_batch_size',
        type=int,
        default=100000,
        help='parquet file read rows batch_size',
    )
    parser.add_argument(
        '--emb_batch_size', type=int, default=5000, help='GPU emb batch size'
    )
    parser.add_argument('--emb_dim', type=int, default=256, help='resize embedding dim')
    args = parser.parse_args()
    logging.info(f'args={args}')
    ds_name = args.hf_repo_id.split('/')[-1]
    save_dir = scratch / 'content_embeddings' / f'{args.emb_model}' / f'{ds_name}'
    save_dir.mkdir(parents=True, exist_ok=True)
    repo_parquet_files_lst = get_ds_files(args.hf_repo_id)
    model = SentenceTransformer(args.emb_model)
    for f_idx, f in tqdm(
        enumerate(
            repo_parquet_files_lst[args.hf_files_start_idx : args.hf_files_end_idx + 1]
        )
    ):
        hf_hub_download(
        ...
        parquet_file = pq.ParquetFile(save_dir / f)
        for i, batch in tqdm(
            enumerate(parquet_file.iter_batches(batch_size=args.parquet_batch_size)),
            ...
            embds: dict[str, list[list[Union[str, np.ndarray]]]] = {}
            for emb_batch_idx in tqdm(range(0, len(domains_lst), args.emb_batch_size)):
                batch_emb = model.encode(
                    domains_txt[emb_batch_idx : emb_batch_idx + args.emb_batch_size]
                )
                for emb_idx in range(len(batch_emb)):
                    ...
                    embds[domains_lst[emb_batch_idx + emb_idx]].append(
                        [
                            urls_lst[emb_batch_idx + emb_idx],
                            batch_emb[emb_idx][0 : args.emb_dim],
                        ]
                    )
