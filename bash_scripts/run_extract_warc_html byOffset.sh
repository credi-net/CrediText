...
if [ -z "$2" ]; then
    outputTableName="warc_extract_content_table"
else
  outputTableName="$2"
fi
...
# Run the Spark job
# Local testing: use "$INPUT_DIR/test_warc.txt"
# Cluster / full usage: ""$INPUT_DIR/all_warc_$CRAWL.txt"
...
"$VENV_PATH/bin/spark-submit" \
    ...
    "$PROJECT_ROOT/creditext/cc-scripts/warc_extract_domain_html_byOffset.py" \
    "$INPUT_DIR/${CRAWL}_test_warc_${start_idx}_${end_idx}.txt" \
    "$outputTableName" \
    ...
    while IFS= read -r line; do
        ...
        rm -f "$file_Path"
    done <  "$INPUT_DIR/${CRAWL}_test_warc_${start_idx}_${end_idx}.txt"